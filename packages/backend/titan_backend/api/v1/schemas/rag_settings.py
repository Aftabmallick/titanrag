...
class RAGSettingsUpdate(BaseModel):
    ...
    context_window_strategy: str | None = Field(
        default=None, max_length=50, description="HIERARCHICAL, FLATTENED, etc."
    )
    ...
    cache_cosine_threshold: float | None = Field(
        default=None, ge=0.80, le=1.0, description="Cosine threshold for semantic cache"
    )
    cache_ttl_seconds: int | None = Field(default=None, ge=60, le=604800, description="Cache TTL in seconds")
    system_prompt_override: str | None = Field(
        default=None, max_length=10000, description="Custom system prompt override"
    )
