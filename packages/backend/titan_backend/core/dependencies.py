...
def require_role(*roles: str) -> Callable[..., Any]:
    async def _dependency(current_user: CurrentUser = Depends(get_current_user)) -> CurrentUser:
        if current_user.is_superuser or current_user.role in roles:
            return current_user
        raise AppException(
            message=f"Required role not held: {', '.join(roles)}",
            status_code=403,
...
async def require_platform_admin(
    ...
    if current_user.is_superuser or current_user.role in ("PLATFORM_ADMIN", "OWNER"):
        return current_user
    raise AppException(
...
require_workspace_permission = require_permission
